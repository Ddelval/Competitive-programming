...
a function over a range of inmutable elements with the following complexity:
\begin{itemize}
		\setlength{\itemsep}{2 pt}
		\item \tbf{$\mathcal O(n\log n)$} to build the data structure
		\item \tbf{$\mathcal O(1)$} to answer queries on a range if
				the function is idempotent
		\item \tbf{$\mathcal O(\log n)$} to answer queries on a
				range if the function is not idempotent
		\item \tbf{$\mathcal O(n\log n)$} memory
\end{itemize}
Therefore the sparse table should only be used for idempotent function since
in other scenarios a BIT or a Segment Tree will have a similar performance while
allowing the elements to be modified.
...
of a reversible function $F$ over a range of values. Given an array of elements
$A$ of size $N$:
\begin{itemize}
		\setlength{\itemsep}{2pt}

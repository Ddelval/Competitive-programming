...
int main() {
    ...
    int n, q;
    cin >> n >> q;
    vi dA = readVector<int>(n);
    vi dB = readVector<int>(n);
    vl deltas(n, 0);
    vl accum(n, 0);
    for (int i = 0; i < n; ++i) {
        deltas[i] = dB[i] - dA[i];
    ...
    echo(accum);
    SparseTable<ll> maxss(accum, [](ll a, ll b) { return max(a, b); });
    while (q--) {
        int a, b;
        cin >> a >> b;
        a--, b--;
        ...
        echo(accum[a]);
        echo(accum[b]);
        if (a > 0 && accum[a - 1] != accum[b]) {
            possible = false;
        } else if (a == 0 && accum[b] != 0) {
            possible = false;
        }
        ll sol = 0;
        if (!possible) {
            ...
            sol = maxss.valueInRange(a, b) - accum[b];
        }
        cout << sol << "\n";
    }

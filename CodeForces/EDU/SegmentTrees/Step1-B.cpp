...
template <ll neutral, ll (*calc)(const ll v1, const ll v2)>
struct SegTree {
    ...
    void recalc() {
        this->val = calc(right->val, left->val);
    }
    ...
    ll rangeSum(int llim, int rlim) {
        if (llim > this->rlim || rlim < this->llim) return neutral;
        if (llim <= this->llim && rlim >= this->rlim) return this->val;
        return calc(right->rangeSum(llim, rlim), left->rangeSum(llim, rlim));
    }
...
ll obtainVal(const ll v1, const ll v2) {
    return min(v1, v2);
}
...
int main() {
    ...
    SegTree<INT_MAX, obtainVal> st(0, n - 1, data);

...
int main() {
    ...
    while (t--) {
        ...
        for (auto cha : s) {
            if (cha == 'R') {
                pll next = make_pair(right.fi + current.fi, right.se + current.se);
                left = current;
                current = next;
            } else {
                pll next = make_pair(left.fi + current.fi, left.se + current.se);
                right = current;
                current = next;
            }
        }
        cout << current.fi << "/" << current.se << "\n";
    }
